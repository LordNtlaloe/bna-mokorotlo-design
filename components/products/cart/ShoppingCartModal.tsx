...
const CartModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className="relative z-10 ">
      <DialogBackdrop className="fixed inset-0 bg-[#0D0D0D] bg-opacity-75 transition-opacity duration-500 ease-in-out" />
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
            <DialogPanel className="pointer-events-auto w-screen max-w-md transform transition duration-500 ease-in-out sm:duration-700">
              <div className="flex h-full flex-col overflow-y-scroll bg-gradient-to-b from-black to-[#0D0D0D]  shadow-xl">
                <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                  <div className="flex items-start justify-between">
                    <DialogTitle className="text-lg font-medium text-gray-100">Shopping Cart</DialogTitle>
                            ...
                            <div className="ml-4 flex flex-1 flex-col">
                              <div>
                                ...
                                <div className="flex items-center mt-2">
                                  ...
                                  <span className="text-gray-200 mx-2">{quantities[index]}</span>
                                  <button
                                    className="text-gray-500 focus:outline-none focus:text-gray-300"
                                    onClick={() => handleQuantityChange(index, quantities[index] + 1)}
                                  >
                              ...
                              <div className="flex flex-1 items-end justify-between text-sm">
                                <button
                                  ...
                                  className="font-medium text-[#F20707] hover:text-red-900"
                                >
                                  Remove
                                </button>
                              </div>
                ...
                <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                  ...
                  <p className="mt-0.5 text-sm text-gray-200">Shipping and taxes calculated at checkout.</p>
                  <div className="mt-6">
                    <button
                      onClick={handleCheckout}
                      className="flex items-center justify-center rounded-md border border-transparent bg-[#F20707] hover:bg-red-900 px-6 py-3 text-base font-medium shadow-sm text-white"
                    >
                      Checkout
                    </button>
                  </div>
